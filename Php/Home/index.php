...
// 读取支付配置
$AdminPayment = array();
$paymentConfigPath = './JCSQL/Admin/Basic/AdminPayment.php';
if (file_exists($paymentConfigPath)) {
    $AdminPayment = json_decode(file_get_contents($paymentConfigPath), true);
}
$tpl->assign('AdminPayment', $AdminPayment);
$payment_enabled = isset($AdminPayment['payment_enabled']) ? $AdminPayment['payment_enabled'] : '0';
$tpl->assign('PaymentEnabled', $payment_enabled);
$tpl->assign('PaymentPrice', isset($AdminPayment['payment_price']) ? $AdminPayment['payment_price'] : '9.90');
$tpl->assign('FreeSeconds', isset($AdminPayment['free_seconds']) ? $AdminPayment['free_seconds'] : '15');
//样式路径
