...
<head>
    ...
    <meta name="description" content="YUTUCMS 添加头部横幅广告">
    <meta name="keywords" content="YUTUCMS, 广告管理, 添加广告">
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="breadcrumb">
            ...
            <a href="#">广告设置</a>
            <span class="separator">/</span>
            <a href="?Php=Home/Ad/Top">头部横幅广告</a>
            <span class="separator">/</span>
            <span class="current">添加广告</span>
        </div>

        <!-- 页面标题 -->
        <h1 class="page-title">
            <i class="fa fa-plus-circle"></i> 添加头部横幅广告
        </h1>
        <p class="page-subtitle">创建新的头部横幅广告</p>

        <!-- 设置表单 -->
        <div class="settings-card">
            <div class="card-header">
                <h2 class="card-title">
                    <i class="fa fa-ad"></i> 广告信息
                </h2>
            </div>
            <div class="card-body">
                <form method="post" class="settings-form">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="TopId" value="" placeholder="请输入广告排序编号">
                        <div class="form-hint">数字越小排序越靠前</div>
                    </div>

                    <div class="form-group">
                        <label class="form-label">广告链接URL <span style="color: var(--danger-color);">*</span></label>
                        <input type="text" class="form-control" name="TopWebUrl" value="" placeholder="请输入广告跳转链接" required>
                        <div class="form-hint">用户点击广告后跳转的目标网址</div>
                    </div>

                    <div class="form-group">
                        <label class="form-label">广告图片URL <span style="color: var(--danger-color);">*</span></label>
                        <input type="text" class="form-control" name="TopPicUrl" value="" placeholder="请输入广告图片链接" required>
                    ...
                    <div class="form-row">
                        <div class="form-col">
                            <label class="form-label">广告图片宽度 <span style="color: var(--danger-color);">*</span></label>
                            <input type="text" class="form-control" name="TopPicUrlWidth" value="100%" placeholder="例如: 100% 或 300px" required>
                            <div class="form-hint">支持百分比和px格式，100%代表自适应宽度</div>
                        </div>
                        <div class="form-col">
                            <label class="form-label">广告图片高度 <span style="color: var(--danger-color);">*</span></label>
                            <input type="text" class="form-control" name="TopPicUrlHeight" value="100px" placeholder="例如: 100px" required>
                            <div class="form-hint">支持百分比和px格式，建议使用固定高度</div>
                        </div>
                    </div>

                    <div class="form-group">
                        <label class="form-label">广告到期时间 <span style="color: var(--danger-color);">*</span></label>
                        <input type="text" class="form-control" name="TopState" value="" placeholder="例如: 20241231" required>
                        <div class="form-hint">格式：YYYYMMDD，如20241231代表2024年12月31日</div>
                    </div>

                    <div class="form-group">
                        <label class="form-label">广告费/联系方式 <span style="color: var(--danger-color);">*</span></label>
                        <input type="text" class="form-control" name="TopRemarks" value="" placeholder="例如: 1000元，QQ：123456" required>
                    ...
                    <div class="btn-group">
                        ...
                        <a href="?Php=Home/Ad/Top" class="btn btn-outline">
            ...
            <div class="card-body">
                <div class="form-group">
                    <div style="background: #f0f7ff; border-left: 4px solid var(--primary-color); padding: 15px; border-radius: 4px;">
                        <h4 style="margin-bottom: 10px; color: var(--primary-color);">填写注意事项：</h4>
                        <ul style="color: var(--dark-color); line-height: 1.6;">
                            <li>带 <span style="color: var(--danger-color);">*</span> 的字段为必填项</li>
                            <li>图片尺寸支持百分比和像素单位，如：100% 或 300px</li>
                            <li>到期时间格式为8位数字：YYYYMMDD</li>
                            <li>联系方式建议使用标准格式，便于管理</li>
                            <li>广告排序数字越小，显示位置越靠前</li>
                        </ul>
                    </div>
                </div>
            </div>
    ...
    $(document).ready(function() {
        ...
        // 实时图片预览
        $('input[name="TopPicUrl"]').on('input', function() {
            ...
            if (imageUrl) {
                ...
                previewContainer.html('<div class="no-image"><i class="fa fa-image" style="font-size: 2rem; margin-bottom: 10px;"></i><p>输入图片URL后显示预览</p></div>');
            }
        });

        // 表单验证
        $('form').submit(function() {
            var requiredFields = ['TopWebUrl', 'TopPicUrl', 'TopPicUrlWidth', 'TopPicUrlHeight', 'TopRemarks', 'TopState'];
            var isValid = true;
            var errorMessage = '';
    ...
    if (isset($_POST['submit']) && isset($_POST['TopId']) && isset($_POST['TopWebUrl']) && isset($_POST['TopRemarks']) && isset($_POST['TopPicUrl']) && isset($_POST['TopState']) && isset($_POST['TopPicUrlWidth']) && isset($_POST['TopPicUrlHeight'])) {
        ...
        $TopId = post_input($_POST["TopId"]);
        $TopWebUrl = post_input($_POST["TopWebUrl"]);
        $TopRemarks = post_input($_POST["TopRemarks"]);
        $TopPicUrl = post_input($_POST["TopPicUrl"]);
        $TopState = post_input($_POST["TopState"]);
        $TopPicUrlWidth = post_input($_POST["TopPicUrlWidth"]);
        $TopPicUrlHeight = post_input($_POST["TopPicUrlHeight"]);
